    ...
    , ui(new Ui::mainwindow)
{
                  ...
                  if(text!=""){
                      if(text.indexOf(" ")!=-1){
                           QMessageBox::warning(this,"警告","密码不能包含空格");
                            ...
                            pr->password=text;
                            changeFile(index,1,this,pr->account,text);
        ...
        if(state){
             ...
             if (ret==BalanceDialog::Accepted){
                  ...
                  if(text!=""){
                      ...
                      changeFile(index,2,this,pr->account,QString("%1").arg(pr->balance));
    ...
    connect(ui->actionsearch_user,&QAction::triggered,[=](){//搜索用户
        if(!isconnect){
            QMessageBox::warning(this,"警告","未与服务器连接");
            return;
        }
   ...
   connect(ui->actionsearch_delivery,&QAction::triggered,[=](){//搜索快递
       if(!isconnect){
           QMessageBox::warning(this,"警告","未与服务器连接");
           return;
       }
   ...
   connect(ui->actiondistribute,&QAction::triggered,[=](){//分配快递员
       if(!isconnect){
           QMessageBox::warning(this,"警告","未与服务器连接");
           return;
       }
   ...
   connect(ui->actioncollect_delivery,&QAction::triggered,[=](){//揽件
       if(!isconnect){
           QMessageBox::warning(this,"警告","未与服务器连接");
           return;
       }
   ...
   connect(ui->actionsearch_courierdelivery,&QAction::triggered,[=](){//快递员搜索快递
       if(!isconnect){
           QMessageBox::warning(this,"警告","未与服务器连接");
           return;
       }
   ...
   connect(ui->actionsearch_courier,&QAction::triggered,[=](){//搜索快递员
       if(!isconnect){
           QMessageBox::warning(this,"警告","未与服务器连接");
           return;
       }
...
void mainwindow::on_LoginButton_clicked()
{
    ...
    if(a!=""&&p!=""){
        ...
        if(reply){
            ...
            state=1;
            if(index==0){
                pr=new realUser(a,p,balance,0);
            }
            else if(index==1){
                pr=new manager(a,p,balance,1);
            }
            else
                pr=new courier(a,p,balance,2);
         }else{
...
void mainwindow::updateDUTable()
{
    ...
    //筛选待揽收快递
    QString strFilter="待揽收";
    int index=0;
    for(int i = 0;i<lEvents.size();i++)
    {
        //如果收件人和发件人都不是用户，表格不显示
        if(!(lEvents[i].status==strFilter))
            continue;
...
void mainwindow::on_send_clicked()
{
    ...
    changeFile(0,2,this,pr->account,QString("%1").arg(pr->balance));
    //读管理员的余额
    ...
    changeFile(1,2,this,"admin",QString("%1").arg(c));
    updateUDTable();
