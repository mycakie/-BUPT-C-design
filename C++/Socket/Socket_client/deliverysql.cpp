...
{
    QSqlQuery sql(m_db);
    sql.exec("select count(id) from courier_delivery;");
    quint32 uiCnt = 0;
    while(sql.next()){
...
QList<DeliveryInfo> deliverysql::getPageDelivery(quint32 page,quint32 cnt)
{
    ...
    QSqlQuery sql(m_db);
    QString strsql = QString("select * from courier_delivery order by id limit %1 offset %2").
    ...
    while(sql.next()){
        ...
        info.courier = sql.value(6).toString();
        info.type = sql.value(7).toString();
        l.append(info);
    }
    return l;
...
{
    QSqlQuery sql(m_db);
    QString strSql = QString("update courier_delivery set status  = '%1' where number = '%2' ").
            arg("已签收").
            arg(info.number);
    return sql.exec(strSql);
}
bool deliverysql::updateDCInfo(DeliveryInfo info)
{
    QSqlQuery sql(m_db);
    QString strSql = QString("update courier_delivery set courier  = '%1' where number = '%2' ").
            arg(info.courier).
            arg(info.number);
    return sql.exec(strSql);
}
bool deliverysql::collectDelInfo(DeliveryInfo info)
{
    QSqlQuery sql(m_db);
    QString strSql = QString("update courier_delivery set status  = '%1' where number = '%2' ").
            arg("已揽收").
